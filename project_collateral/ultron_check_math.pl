...
open(MATHFILE,">check_math.dat") || die "ERROR: can't open check_math.dat for write";
...
for ($calcs=0; $calcs<($set_cntr+1)/6; $calcs++) {
  ...
  printf MATHFILE "// rht and lft value for cycle %d \n",$calcs;
  $base = $calcs*6;
  $accum = 0;
  $accum+=$a2d[$base+0][1];
  $accum-=$a2d[$base+1][0];
  ...
  my $s_rht_reg =  sprintf("%x", $rht_reg);
  if (length($s_rht_reg) <= 4){
  printf MATHFILE "\@%x %04x\n", 2* $calcs, $rht_reg;
  }
  else{
  $sub_s_rht_reg = substr($s_rht_reg, -4);
  printf MATHFILE "\@%x %s\n", 2* $calcs, $sub_s_rht_reg;
  }
  $accum = $Fwd + $Pcomp;
  ...
  printf SUMMARY "%3x\n",$lft_reg;
  my $s_lft_reg =  sprintf("%x", $lft_reg);
  if (length($s_lft_reg) <= 4){
  printf MATHFILE "\@%x %04x\n", 2* $calcs + 1, $lft_reg;
  }
  else{
